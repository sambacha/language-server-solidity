 ...
 */
export function execShellScript(body: string, cmd = 'solidity'): Promise<string> {
  const args = ['-c', body];
...
export async function getShellDocumentationWithoutCache({
  ...
  if (word.split(' ').length > 1) {
    throw new Error(
      `lookupDocumentation should be given a word, received "${word}"`
    );
  }
...
export const getShellDocumentation = memorize(
